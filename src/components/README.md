...
| | |
| --- | --- |
| `AppPage.tsx` | One app's own page. **Knows nothing about any particular app** — everything comes from `data/appPages.ts`. The one exception is derived, not typed: it asks `STORE_APPS` whether this app sells anything and draws the way on to its packs opposite the Back control, so an app that starts selling needs no edit here or in its prose (rule 17). |
...
| `Store.tsx` | The shop, in **two views over one set of state**: `#/store` is an index of one card per app, `#/store/<app>` is that app's own page of packs with the buy and manage panels on them. Both open and close with the app pages' own `BackButton`, and `#/store/<app>` pairs it with an `OnwardButton` into that app's prose — the exact mirror of the control that app's page offers into its packs. Read it with `data/store.ts` and `store/useOwnedPacks.ts` — see below. The index opens with the TDG Cloud shelf, which lives in [`src/cloud/`](../cloud/README.md) because it is a whole surface, not a card. |
...
| | |
| --- | --- |
| `Nav.tsx` | The bar, the theme toggle, the account menu (identity, **Finish Setting Up** while a provider account is still missing a username or a password, **Choose a New Password** while a reset link was opened and dismissed without one being set, Send Feedback, the Developer Tab switch, sign out), and the Developer tab that appends itself for a signed-in developer with Developer Mode on. That tab is **not** in `NAV_LINKS`, because that array is the public navigation and is read by everything. Pressing the mark still goes home, and also turns the cross a quarter turn into a sword and rolls *Jesus Loves You* out along the line it points down, one letter at a time (each `.nav__bless-letter` carries `--k`, its index across the phrase, and Nav.css spends it as a 38ms stagger), for five seconds; how much bar it has is measured, so on a full bar the phrase either shrinks to fit or the link row dims for it. The words are white and gilded in both themes — a gold rim and glow from `--warm`, bronze-rimmed on the light theme so white reads on the cream veil — and each letter also carries `data-ch`, its own character again, which Nav.css draws over it as a gold shine that runs along the phrase on the same 38ms beat. Every moving part is transform or opacity, and the sword's two drop-shadows keep one radius each and fade by colour, because an animated radius compiles a GPU shader per value and that was the stutter. |
| `Footer.tsx` | |
...
| `ErrorBoundary.tsx` | The face a page gets when it cannot be drawn — above all a lazy chunk that is no longer on the server after a deploy, which would otherwise unmount the root and leave a blank page. Every routed page in `App.tsx` renders inside one, keyed on the route; the hero's point cloud and the walk's cabin use its `silent` form and simply go missing from their scene. A Reload button rather than an automatic reload, because on GitHub Pages a cached `index.html` can make an automatic one loop. |
