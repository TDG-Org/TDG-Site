...
/**
 * A perspective-projected point field behind the Origin timeline. Same
...
export function OriginField() {
  ...
  useEffect(() => {
    ...
    /*
     * ── why anything that changes the canvas has to ask for a frame ────────
     *
     * At motionIntensity 0 this subscriber paints exactly one frame and then
     * returns before `clearRect` on every frame after it — which is the point:
     * a visitor who asked for less motion gets a still field, and the loop
     * gets to park.
     *
     * The cost is that `settled` is a claim about what is currently ON the
     * canvas, and three things can falsify it without the loop being able to
     * see any of them. A resize writes `cv.width`, and writing a canvas's
     * width blanks it, so a reduced-motion visitor who resized the window —
     * or dragged it between two monitors with different scaling, which is the
     * same thing through onDprChange — lost the entire field for the rest of
     * the visit with no code path left that could repaint it. A theme swap is
     * the milder version of the same bug: the colours were re-read and nothing
     * ever painted with them.
     *
     * So each of the three invalidates the claim and wakes the loop for one
     * more frame. `wake()` exists for exactly this — see lib/motion.ts.
     */
    let settled = false
    const invalidate = () => {
      settled = false
    ...
    const readColors = () => {
      ...
      warmRGB = readRGB(section, '--origin-glow-warm', warmRGB)
    }
    readColors()
    ...
    const resize = () => {
      ...
      // the two lines above just blanked the canvas
      invalidate()
    }
    resize()
    ...
    const stop = onFrame(({ vh, mi, dt, now, hold }) => {
      ...
      //
      // The chapters fold shut now, so the section's resting height dropped by
      // roughly a quarter. The 1.4 is deliberately left where it is: it is the
      // TOTAL turn across one traversal, ~80°, and shortening the run does not
      // change that — it only spends it over less scrolling (about 26° per
      // screen before, about 31° now). Both read as a slow deliberate turn,
      // and the constant is what the field was tuned at. Opening a chapter
      // grows `r.height` mid-read and steps ry by about two degrees on 130
      // points, which is below what this field can show.
  ...
  return <canvas ref={canvas} className="origin__field" aria-hidden="true" />
}
