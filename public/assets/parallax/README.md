...
| `props/window-frost` | `props/capiz-window` | The walk's front stage — **dark only** since 2026-09-05. The frost is a vignette by construction and this file is not: its ink covers the whole canvas with a cross cut through it, so even masked to the edges at 0.66 it filled the Tools band with blurred brown panes and the three cards floated on them. The owner took it off in the Scene Editor. The winter frost is untouched and is still what makes that walk read as being seen from inside |
...
| `landscapes/far-range-soft` | `landscapes/headland` | **#faith**, the headland across the water at the foot of the section, placed in the Scene Editor on 2026-09-05 (`.faith__headland`). It was the Outro's far shore until the clearance below took it off that beat; this is the piece finding a place rather than being deleted, which is what this README asks for two sections down. Light only — a green hill across a lagoon is not in the midnight picture |
...
| `landscapes/snow-bank` | `landscapes/beach-terrace-plain` | Outro, the sand. **Not drawn in light** since 2026-09-05: the clearance below left it as the last piece of art in the light Makers beat and the owner then took that too, so what is left there is the band, the afterglow and `.outro__ground`'s pool of light under the copy |
