...
(`ddbksawvchsauiuiwvrl`) — **with one exception, marked in the table below and
in its own header.** A file that is written but not yet applied is the one thing
this folder cannot be trusted about at a glance, so it says so in both places
until somebody runs it. They are kept because the alternative, schema that
