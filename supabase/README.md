...
| Path | What it is |
| --- | --- |
| `functions/tdg-site-account/index.ts` | Turns "username **or** email + password" into a session, and sends a password-reset link for either. A 5xx from Auth is answered as `server_error` 502, never dressed as wrong credentials. |
...
| `functions/cloud-stripe-webhook/index.ts` | Stripe → TDG Cloud plan ownership: writes `cloud_entitlements.grants` from checkout and subscription events, stamps the billing cadence into the grant, and maps live price ids to packs so a portal plan change between Standard and Studio moves the grant. Deployed with `--no-verify-jwt`; the refetch is the authentication, exactly like the veditor and devfleet webhooks it is a sibling of. Writes `stripe_customer_id` once and logs a second payer rather than overwriting the first; a `client_reference_id` that names no account (an edited buy URL, a deleted account) falls through to the payer's email via `cloud_user_exists` instead of a 500 Stripe retries for days; a subscription's owner comes from `cloud_user_for_subscription`, not a 1000-row-capped scan; a ledger row refused for a dead `user_id` is written unattributed rather than not at all. Since `@3` its one write is `cloud_apply_grant` in Postgres, so a pack a developer has revoked lands in the block's held copy and never back on the live row — see *Taking a product away*. |
| `functions/cloud-storage/index.ts` | The ONE door between TDG Cloud clients and the Backblaze B2 bucket. Forwards the caller's own JWT into the Postgres gates (`begin_upload`/`begin_download`), then answers S3-presigned URLs — single PUT or multipart — so bytes move client↔B2 directly; verifies each landed object with its own HEAD before booking it; refuses to book more than the reservation promised (and releases that spent reservation rather than leaving it to hold quota until its TTL); deletes every version (gone, not hidden — and the version listing pages, so a name rewritten a hundred times still deletes whole); hands out multipart part URLs only against a live reservation and only for the parts its size can hold; an object that lands with no reservation behind it is not kept unless the catalogue already books exactly those bytes (a retried finish, answered `already_booked`); a refused upload loses only its newest version, so the file it was replacing stays current; `upload-cancel` removes a single PUT that already landed. Credential from Vault via `tdg_cloud_b2_credentials()`. Deployed `--no-verify-jwt` (it does its own auth and answers CORS). |
...
| `functions/cloud-maintenance/index.ts` | The deliberate arm of Cloud retention: the lapsed-accounts report, expired-reservation reaping, and — double-gated on the caller being a developer AND `availability.auto_purge` in config — the purge, which destroys every B2 object version under the account's prefix and settles the books in one verb. Dry-run by default. Also `bucket`: Backblaze's own listing held against the catalogue — orphans B2 bills us for that no account claims, ghosts the catalogue promises that are not there, size mismatches, and the old-version/hide-marker tax. Its developer check goes through `tdg_cloud_is_developer()`, NOT a read of `profiles` (`service_role` has no SELECT there, which silently refused every developer until 20260831234500). Answers `OPTIONS` with 204 and CORS so the console can call it from a browser; a purge checks that each account's books settled (500 when any did not) and its audit line names the developer who pressed it; the retention anchor is the one `tdg_cloud_plan_of` uses — last period end, else when the grants changed, else when the bytes changed, never `now()`; every catalogue read pages past PostgREST's 1000-row cap. |
...
| `functions/tdg-store-verify/index.ts` | Answers whether Stripe still agrees with what the Store advertises: every app-tagged payment link's real amount, cadence and active state; the Cloud config held against Stripe in both directions (a configured link must exist, sell exactly the configured cents, and be active if and only if Cloud is on sale); and the three app webhook endpoints' health. `npm run verify:store` reads the catalogue out of `store.ts` and POSTs it for per-link verdicts. Deployed `--no-verify-jwt` for the `tdg-site-deploys` reason: no identity in, nothing non-public out — the Stripe key stays in the environment and only its conclusions leave. The subscription endpoints must also carry `customer.subscription.deleted`, the event that turns an ended plan into a lapsed grant. Since `@4`, GET and POST share an upstream snapshot under a minute old (`X-Sweep-Cached: true`, `checkedAt`) and coalesce concurrent sweeps within an instance, limiting anonymous polling against the Stripe account the webhooks also read. POST compares its own catalogue against those cached facts; caller verdicts are never cached. Failed or malformed Cloud configuration reads fail the gate instead of reporting success. Since `@3`, each catalogue link's Stripe `metadata.app`/`pack` must match the app and pack the site sells it under, so equal prices cannot make swapped links pass. |
| `public.tdg_purchase_events` | One ledger of every Stripe event that granted anybody anything, across every TDG app — the 2026-09-01 merge of `cloud`/`devfleet`/`veditor_purchase_events` and `mak_subscription_events`. Those four names live on as compatibility views with INSTEAD OF INSERT triggers, because three of their writers are webhooks deployed from other repositories. |
...
**`tdg_public_stats()` is one of the three functions on this project granted to `anon`** (with `tdg_site_content()` and `tdg_cloud_public_config()`, each identity-free for the same reason),
...
`tdg_my_revocations()` is what an account and its apps read, and since
20260901130000 it carries `held_grants` — the grants the block took — so the
Store's revoked card can say a subscription is still renewing underneath and
draw the one button that stops it; `tdg_billing_subscription` reads under the
block the same way, or Manage or Cancel Plan would have answered "no
subscription" to somebody still being charged. The table also has
...
**A subscription event that arrives while its pack is blocked lands under the
block.** Stripe keeps raising `customer.subscription.*` events for a revoked
pack's subscription until somebody cancels it, and until 20260901150000 the
Cloud webhook resolved such an event only against the live grants and then
upserted straight onto the live row — so the first renewal after a revocation
put the pack back in force, inside TDG Cloud itself (`cloud_packs_in_force()`
reads that row), not only on the Store's card. Now every write the webhook
makes goes through `cloud_apply_grant`, which asks `tdg_product_revocations`
first: a pack under its own row, or under the whole-app `*` row, has Stripe's
latest word written into that row's `held_before` and nothing written to the
live row (a stale live copy under a block is taken off). `cloud_user_for_subscription`
looks under the block too, so the renewal is still that account's in the
ledger — money taken is money taken, revoked or not. Lifting the block then
hands back the *current* grant, the latest period end and status, rather than
a snapshot from the day of the block. A payment link used by a revoked account
lands the same way: recorded, held, never in force. Proven on the live project
in a rolled-back block, 52 checks, before the migration was applied. The
Veditor webhook, deployed from its own repository, still upserts and needs the
same treatment.
